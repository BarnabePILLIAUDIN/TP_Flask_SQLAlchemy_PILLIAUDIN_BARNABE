...
from .constants import SEED_TYPE_OF_ROOMS, STATUS_OF_BOOKING
...
def create_seed_clients(number_of_clients=config["max_number_of_seeds"]):
...
def create_seed_rooms(number_of_rooms=config["max_number_of_seeds"]):
...
def create_seed_booking(rooms, clients):
    ...
    return Booking(
        ...
        status=get_random_element_of_list(STATUS_OF_BOOKING)
    )


def create_seed_bookings(number_of_bookings=config["max_number_of_seeds"]):
